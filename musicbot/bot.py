...
class MusicBot(discord.Client):
    ...
    async def _cmd_play_playlist_async(self, player, channel, author, permissions, playlist_url, extractor_type):
        ...
        busymsg = await self.safe_send_message(
            channel, "Adding %s songs..." % num_songs)  # TODO: From playlist_title
        await self.send_typing(channel)
    ...
    async def cmd_search(self, player, channel, author, permissions, leftover_args):
        ...
        for e in info['entries']:
            result_message = await self.safe_send_message(channel, "Result %s/%s: %s" % (
                info['entries'].index(e) + 2, len(info['entries']), e['webpage_url']))

            confirm_message = await self.safe_send_message(channel, "Type `y`, `n` or `exit`")
            response_message = await self.wait_for_message(30, author=author, channel=channel, check=check)

            if not response_message:
                await self.safe_delete_message(result_message)
                await self.safe_delete_message(confirm_message)
                return Response("Didn't find it .", delete_after=30)
    ...
    async def cmd_summon(self, channel, author, voice_channel):
        ...
        await self.safe_send_message(channel, "Joined {}! ".format(author.voice_channel))
        if player.is_stopped:
            player.play()
    ...
    async def cmd_shuffle(self, channel, player):
        ...
        em = discord.Embed(description = "Shuffled Playlist! :D")
    ...
    async def cmd_disconnect(self, server):
        ...
        #em = discord.Embed(description = "Leaving! :D")
        return Response("Leaving :wave::smile:", delete_after=20)
        #await self.send_message(channel, embed =em)
